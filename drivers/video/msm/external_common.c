...
struct msm_hdmi_mode_timing_info
	...
	HDMI_VFRMT_720x576p50_4_3_TIMING,
...
// ASUS_BSP +++ Tingyi "[A80][MyDP] Support Pad resolution"
    VFRMT_NOT_SUPPORTED(HDMI_VFRMT_1280x800p60_16_10),
    VFRMT_NOT_SUPPORTED(HDMI_VFRMT_1920x1200p60_16_10),
// ASUS_BSP --- Tingyi "[A80][MyDP] Support Pad resolution"
};
...
static int hdmi_common_read_edid_block(int block, uint8 *edid_buf)
{
	...
	int status;
retry:
	status = external_common_state->read_edid_block(block, edid_buf);
	if (status)
		goto error;
